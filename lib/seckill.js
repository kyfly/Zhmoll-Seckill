...
class seckill_cache {
  constructor(seckillid, startTime, io) {
    ...
    this.token_count_set_refresh = setInterval(_refresh, config.seckill.checkTime, this, seckillid);
    // 用于销毁本次秒杀活动的缓存
    const countdown = this.start - Date.now();
    this.destory = setTimeout(_destory, countdown + config.seckill.cachePreserveLimit, seckillid);
    this.persist = setTimeout(redis.Seckill.persist, countdown + config.seckill.persistAfterStart, seckillid, this._attend_count, this._click_count);
  }
  emit_to(socket, msg) {
...
function _refresh(that, seckillid) {
  that.emit();
  const set = that.token_count_set;
...
function onAuth(io, socket, data) {
  (async () => {
    ...
    .then(() => {
      ...
      // 添加活动结束断连倒计时
      countdown = seckill(socket.seckillid).start - Date.now() + config.seckill.allowLoginRight;
