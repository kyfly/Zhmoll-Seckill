...
function _kill_cheat_msg(e, seckillid, token, socket) {
  ...
  redis.Seckill.turnBackAward(seckillid, token, e.message)
    .then(award => {
      ...
      // 30s秒后断连
      setTimeout(function (socket) { socket.disconnect(true); }, 30000, socket);
    })
...
function validate5seconds(socket) {
  return _validate(socket).catch(e => {
    logs.info('五秒验证未通过', { token: socket.token, seckillid: socket.seckillid });
    seckill_set[socket.seckillid].emit_to(socket, '你已在别处登录！', e.message);
