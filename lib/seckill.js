...
class seckill_cache {
  constructor(seckillid, startTime, io) {
    ...
    // 用户缓存用户上一次提交秒杀请求的时间
    this.token_click_last_time_set = {};
    // 从缓存数据库中读取奖品余量
  ...
  decre_token_count(token) {
    ...
    --this.token_count_set[token];

    // 上次点击时间判断 针对短时间快速的请求
    if (this.token_click_last_time_set[token]) {
      const countdown = now - this.token_click_last_time_set[token];
      if (countdown < config.seckill.pms)
        --this.token_count_set[token];
    }

    // 更新成为本次点击时间
    this.token_click_last_time_set[token] = now;
    if (this.token_count_set[token] < 0)
...
function onConnect(io, socket) {
  ...
  _checkSockillidExisted(seckillid, io)
    ...
    .catch(e => {
      ...
      // 缓存未建立，不能使用这种方法来发送信息，换一种方法发送
      // seckill(seckillid).emit_to(socket, '连接后缓存建立产生错误', e.message);
      socket.send({ t: Date.now(), m: '连接后缓存建立产生错误', e: e.message })
