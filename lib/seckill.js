...
class seckill_cache {
  constructor(seckillid, startTime, io) {
    ...
    //   countdown + config.seckill.persistAfterStart, seckillid, this._click_count);
    // 活动允许时间右界关闭连接
    this.closeConnections = setTimeout(_closeConnections,
      countdown + config.seckill.allowLoginRight, this);
    // 用于销毁本次秒杀活动的缓存
...
function handShake(request, cb) {
  ...
  // 0、检验数据完整性
  if (!seckillid || !token) {
    console.error(new Date(), '握手认证数据不完整');
  ...
  redis.TokenPool.get(seckillid, token).then(socketid_in_db => {
    if (!socketid_in_db) {
      console.error(new Date(), seckillid, token, 'seckillid或Token不存在');
      return cb('TOKEN_INVALID', false);
    }
    // 2、检测是否在黑名单里面
    redis.BlackRoom.exist(token).then(isBlocked => {
      if (isBlocked === 1) {
        console.error(new Date(), seckillid, token, '尝试连接封禁的Token');
        return cb('CHEAT_TOKEN_FORBIDDEN', false);
      }
...
function onConnect(io, socket) {
  ...
  if (io.sockets.connected[socketid_in_db]) {
    console.info(new Date(), seckillid, token, '关闭同时进行的Websocket连接');
  ...
  _checkSockillidExisted(seckillid, io)
    ...
    .catch(e => {
      console.error(new Date(), seckillid, token, '连接后缓存建立产生错误');
    ...
    .catch(e => {
      ...
      redis.Seckill.turnBackAward(seckillid, token)
        .then(award => {
          ...
          console.error(new Date(), socket.seckillid, socket.token, '作弊检测未通过', e.message);
          socket.disconnect(true);
        });
    });
}
...
function validate5seconds(socket) {
  ...
  (async () => {
    ...
    .catch(e => {
      console.error(new Date(), seckillid, '五秒验证未通过', token);
