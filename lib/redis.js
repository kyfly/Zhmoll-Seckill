...
function _persisted(seckillid, attendCount, clickCount) {
  ...
  redis.hgetall('Seckill_AwardList#' + seckillid, (err, result) => {
    // 拿到所有秒杀获奖名单
    length = result.length / 2;
    for (let i = 0; i < length; i++) {
      // 拆分到个人token，谁拿到了什么奖品
      const token = result[2 * i];
      const award = JSON.parse(result[2 * i + 1]);
      Token.find({ token: token, seckillid: seckillid }, function (err, token) {
        // 找到token的拥有者，创建结果
        const userid = token.userid;
        SeckillResults.create({ userid: userid, seckillid: seckillid, content: award }).exec();
      });
    }
  });
