...
redis.defineCommand('fetchAwardOnce', {
  numberOfKeys: 2,
  lua:
  ...
  if (award==nil) then
    ...
    redis.pcall("HSET", KEYS[1], ARGV[1], award)
    return award
  end
end`
});

redis.defineCommand('turnBackAward', {
  numberOfKeys: 2,
  lua:
...
async function _putAwardIntoSeckill(seckillid, award) {
  return await redis.rpush('Seckill_AwardPool#' + seckillid, JSON.stringify(award));
}
async function _fetchAward(seckillid, token) {
  return await redis.fetchAwardOnce('Seckill_AwardList#' + seckillid, 'Seckill_AwardPool#' + seckillid, token);
}
async function _turnBackAward(seckillid, token) {
  return await redis.turnBackAward('Seckill_AwardList#' + seckillid, 'Seckill_AwardPool#' + seckillid, token);
}
async function _countRestAwardInSeckill(seckillid) {
  return await redis.llen('Seckill_AwardPool#' + seckillid);
}
redis.Seckill = {
